...
- La imagen contiene texto y cumple una función, por ejemplo un ícono de un botón. Ese ese caso el alt *debería comunicar la función de la imagen*.
- La imagen es usada en un link o un botón y es imposible de decir que hará este elemento si la imagen no está. En este caso, el alt *debería comunicar la página de destino (en caso de ser o link) o función (en caso de ser un botón)*
- La imagen tiene texto y este **no** está presente en otro lado del sitio. En este caso el alt *debería tener el texto que está dentro de la imagen*.
- La imagen es un gráfico que muestra información que de otro modo haría difícil de entender el contenido del sitio. En este caso el alt *debería contener una breve explicación sobre el contenido de la imagen*.
...
Para lograr el estándar AAA de accesibilidad, es importanter que las imágenes que contienen texto *sólo se usen para decoración o cuando su uso es esencial para mostrar la información de la que se quiere hablar*
...
|Objetos gráficos y elementos interactivos|*3:1*  |*3:1*  |
...
- Ante la duda de si algo se ve bien o no para las personas con distintos grados de daltonismo, se puede usar la pestaña de **Rendering** de DevTools. Para verla, una vez abierto DevTools se hace clic en los 3 puntos a la parte derecha, luego a *Más herramientas* y luego se selecciona *Rendering*.
  ...
  En este modo se puede ir a la opción de *Emular deficiencias de color* y seleccionar cualquiera de los filtros en la parte de *Emular deficiencias de visión*. El siguiente screenshot muestra como se vería una página con el filtro de *Deuteranopia*
...
Para cumplir el nivel de accesibilidad AAA, todos los elementos *funcionales* del sitio deben ser navegables por teclado sin requerir tiempos específicos de uso de combinaciones de teclas.
...
Por estas razones deberíamos hacer lo posible para *no* usar estos generadores de íconos en los proyectos.
...
```html
<p>El contenido del sitio está normalmente en español <span lang="en">but this part of the site is in English</span></p>
```
